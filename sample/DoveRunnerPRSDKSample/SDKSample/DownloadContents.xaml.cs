    ...
    public sealed partial class DownloadContents : Page
    {
        ...
        private async void StartPlayback(ContentInfo info)
        {
            try
            {
                ...
                info.DownloadPlayUrl = await PRSDKWrapper.GetPlayBackUriAsync(info);
                await PRSDKWrapper.SetPlayReady(mediaElement, info);
                PRSDKWrapper.SetSoftware(mediaElement);
                await PRSDKWrapper.GetLicenseAsync(info);
        ...
        private async void StartDownload(ContentInfo info)
        {
            try
            {
                if (info.IsDownloading == false)
                {
                    ...
                    if (callbackRequest == null)
                        await PRSDKWrapper.DownloadTaskAsync(info, null, DownloadProgress, DownloadComplete, DownloadFail);
                    else
                        await PRSDKWrapper.DownloadTaskAsync(info, callbackRequest, DownloadProgress, DownloadComplete, DownloadFail);

                    await PRSDKWrapper.DownloadStart(info);
                }
                else
                {
            ...
            {
                PRSDKWrapper.DownloadCancel(info);
                ContentDelete(info);
                System.Diagnostics.Debug.WriteLine(ex.Message);
        ...
        private async void DownloadResume(ContentInfo info)
        {
            try
            {
                ...
                {
                    info.IsDownloading = true;
                    await PRSDKWrapper.DownloadResume(info);
                }
                else
                {
                    MessageDialog dialog = new MessageDialog("donwloading...");
        ...
        private async void SetPlayerSubtitle(Uri mpdUri, ContentInfo content)
        {
            try
            {
                ...
                var subtitleLists = await PRSDKWrapper.GetSubtitleLIst(mpdUri, content);
        ...
        private async void ListView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                if (info.IsDownloading == true)
                {
                    ...
                    PRSDKWrapper.DownloadPause(info);
        ...
        private void DownloadProgress(string contentName, int index, int totalIndex)
        {
            if (!stopUpdating)
            {
                ...
                PRSDKWrapper.Logger(contentName + "  Total:" + totalIndex.ToString() + "  Download Index:" + index.ToString());
            }
        }
